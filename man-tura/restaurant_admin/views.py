...
class LoginAdmin(TemplateView):
    '''Для отображения страницы с логином для  админа'''
...
# Для отображения страницы с выбором задачи
...
# Для отображения страницы для управления заказами
...
# Для отображеня страницы для изменения и нформации про ресторан
def rest_info_editing_page(request):
    rest_info_list = RestaurantInfo.objects.all()
    context = {
        'title':'Редагування інформації про ресторан',
        'path_pref':'../../',
        'resta_info': rest_info_list
    }
    if request.method == 'POST':
        rest_info = rest_info_list[0]
        if request.POST.get("button-submit") == "changing-footer":
            rest_info.rest_phone_number = request.POST.get("tel-number")
            rest_info.time_working = request.POST.get("time-working")
            rest_info.wifi_name = request.POST.get("wifi-name")
            rest_info.wifi_password = request.POST.get("wifi-password")
        elif request.POST.get("button-submit") == 'changing-video':
            if 'video' in request.FILES:
                rest_info.main_page_video = request.FILES['video']
        elif request.POST.get("button-submit") == 'changing-about':
            
            rest_info.about_restaurant = request.POST.get("about-text")
